...
system(
    "sort -k1,1 -k2,2n tmp$time/24.sirna.sam.bed >tmp$time/24.sirna.sam.sorted.bed" #removed --parallel=$CPU because of the error message it throws-- sort: unrecognized option '­­parallel=1' Try `sort ­­help' for more information.
);
...
# system("ls tmp$time/*.blast >tmp$time/blastlist.txt");  #put this line in to make a file that contains a list of the blast files so silix compiled t will work use openmpi will work 

# system(
#    "mpirun -np 4 silix -i $idensilix -r $overlapsilix tmp$time/24.sirna.merged.$window.$siRNAs.mask.final.fasta tmp$time/blastlist.txt -f FAM > tmp$time/clusters 2>TASR_log/silix.log"
# ); # # mpirun -np was added to use if  silix was compiled for open mpi. This command expects a file that has a list of blast output files and doesnt take the blast file itself as the input.
...
##########Detect Repeated sequences using a blast all against all######
    ...
    system(
        "usearch -id 0.8 -cluster_fast  tmp$time/clustering/$name.tfa.final.repeat -centroids tmp$time/clustering/$name.tfa.final.repeat.centroid>/dev/null 2>/dev/null"
    );

    if ( $blastv eq 'NCBI-BLAST' ) {
        system(
            "formatdb -i tmp$time/clustering/$name.tfa.final.repeat.centroid -p F >TASR_log/formatdb.blast.log"
        ); #removed .centroid from *tfa.final.repeat.centroid  because the previous step outputs *tfa.final.repeat not tfa.final.repeat.centroid  
        system(
...
sub REPEAT {
    ...
    close(OUTPUT);
    system(
        "sort -k1,1 -k2,2n $filename.bed > $filename.bed.sorted" # --parallel=$CPU was removed as it is not an option for sort and the progran was throwing an error
    );
